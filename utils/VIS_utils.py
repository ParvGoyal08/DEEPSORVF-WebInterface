...
"""Detection and tracking utilities (CPU forced to avoid CUDA driver issues)."""
...
class VISPRO(object):
    def __init__(self, anti, val, t):
        ...
        # Initialize YOLO & DeepSort on CPU
        cfg = get_config()
        cfg.merge_from_file("deep_sort/configs/deep_sort.yaml")
        self.yolo = YOLO(cuda=False)
        ...
        print('[VISPRO] Initialized YOLO + DeepSort on CPU.')

    def detection(self, image):
        """Run vessel detection and return bounding boxes."""
        im0 = cv2.cvtColor(image, cv2.COLOR_BGR2RGB)
        im0 = Image.fromarray(im0)
        return self.yolo.detect_image(im0)
        ...
        if timestamp % 1000 < self.t:
            ...
            if timestamp % 1000 < self.t:
                ...
                for i in self.OAR_ids_list:
                    self.Anti_occlusion_traj = pd.concat([self.Anti_occlusion_traj, self.VIS_tra_last.iloc[id_list.index(i)].to_frame().T], ignore_index=True)
