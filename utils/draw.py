...
def process_img(df_draw, x1, y1, x2, y2, fusion_current, w, h, w0, h0, Type):
    ...
    if Type:
        ...
        lat  = -1
        lon  = -1
    df_draw  = pd.concat([df_draw, pd.DataFrame([{'ais':ais,'mmsi':mmsi,'sog':sog,"cog":cog,'lat':lat,'lon':lon,\
...
class DRAW(object):
    def __init__(self, shape, t):
        self.df_draw = pd.DataFrame(columns=['id', 'ais', 'mmsi', 'sog', 'cog',\
                'lat', 'lon', 'box_x1', 'box_y1', 'box_x2', 'box_y2',\
                                    'inf_x1', 'inf_y1', 'inf_x2', 'inf_y2', 'color'])
        ...
        self.t = t
        # Track last drawn boxes and per-ID visibility toggles (False means info hidden)
        self.last_boxes = []
        self.visibility_by_id = {}
        self.verbose = True
    ...
    def set_visibility(self, track_id, visible):
        self.visibility_by_id[int(track_id)] = bool(visible)
        if self.verbose:
            print(f"[DRAW] set_visibility id={int(track_id)} -> {bool(visible)}")

    def toggle_visibility(self, track_id):
        tid = int(track_id)
        cur = self.visibility_by_id.get(tid, False)
        self.visibility_by_id[tid] = not cur
        if self.verbose:
            print(f"[DRAW] toggle_visibility id={tid} now={self.visibility_by_id[tid]}")
    ...
    def draw_traj(self, pic, AIS_vis, AIS_cur, Vis_tra, Vis_cur, fusion_list, timestamp):
        ...
        if timestamp % 1000 < self.t:
            df_draw = pd.DataFrame(columns=['id', 'ais', 'mmsi', 'sog', 'cog',\
                'lat', 'lon', 'box_x1', 'box_y1', 'box_x2', 'box_y2',\
                                    'inf_x1', 'inf_y1', 'inf_x2', 'inf_y2', 'color'])
                ...
                if last != -1:
                    ...
                    if id_current['timestamp'][last] == timestamp//1000 and len(fusion_list) != 0:
                        ...
                        if len(fusion_current) != 0:
                            tmp_df = process_img(pd.DataFrame(columns=df_draw.columns), x1, y1, x2, y2,\
                    ...
                    # 不存在AIS信息
                        ...
                        tmp_df = process_img(pd.DataFrame(columns=df_draw.columns), x1, y1, x2, y2,\
                                      fusion_current, self.w, self.h, self.wn, self.hn, Type = False)      
                    # attach id
                    tmp_df['id'] = int(id_current['ID'][last])
                    df_draw = pd.concat([df_draw, tmp_df.iloc[[0]]], ignore_index=True)
            self.df_draw = filter_inf(df_draw, self.w, self.h, self.w0, self.h0, self.wn, self.hn, self.tf)
            # cache last boxes for hit-testing
            self.last_boxes = []
            for _, row in self.df_draw.iterrows():
                self.last_boxes.append({
                    'id': int(row['id']),
                    'box': (int(row['box_x1']), int(row['box_y1']), int(row['box_x2']), int(row['box_y2'])),
                    'has_ais': int(row['ais']) == 1
                })
        # 画标识
        
        add_img = self._draw_with_visibility(add_img, self.df_draw, self.tf)
        return add_img

    def _draw_with_visibility(self, add_img, df_draw, tf):
        length = len(df_draw)
        if length != 0:
            y1 = df_draw['box_y2'][0]
            y2 = df_draw['inf_y1'][0]
        ...
        for ind, inf in df_draw.iterrows():
            ...
            inf_y2 = inf['inf_y2']
            color  = inf['color']
            track_id = int(inf['id']) if 'id' in inf else -1

            add_img = draw_box(add_img, box_x1, box_y1, box_x2, box_y2, color, tf)

            show_info = self.visibility_by_id.get(track_id, False)
            if show_info:
                if int(ais) == 1:
                    cv2.rectangle(add_img, (inf_x1,inf_y1), (inf_x2,inf_y2),\
                                  color, thickness=tf//3, lineType=cv2.LINE_AA)
                    ...
                    i = i + 1
                    if self.verbose:
                        print(f"[DRAW] draw AIS info for id={track_id} mmsi={mmsi}")
                else:
                    cv2.rectangle(add_img, (inf_x1,inf_y1), (inf_x2,inf_y2),\
                                  color, thickness=tf//3, lineType=cv2.LINE_AA)
                    add_img = draw_line(add_img, (box_x1+box_x2)//2, box_y2, (inf_x1+inf_x2)//2, inf_y1, y*(i+1)//(length+1), color, tf)
                    ...
                    i = i + 1
                    if self.verbose:
                        print(f"[DRAW] draw NO AIS panel for id={track_id}")
            else:
                if self.verbose:
                    print(f"[DRAW] hide info panel for id={track_id}")
