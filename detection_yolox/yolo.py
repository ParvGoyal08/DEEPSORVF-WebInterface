...
class YOLO(object):
    _defaults = {
        ...
        #   没有GPU可以设置成False
        #-------------------------------#
    #   Default to CPU for broader compatibility; enable explicitly if GPU + driver available
    "cuda"              : False,
    }
    ...
    #---------------------------------------------------#
    def generate(self):
        # Decide device: only enable cuda if requested AND available.
        if self.cuda and not torch.cuda.is_available():
            print('[YOLO] CUDA requested but not available. Falling back to CPU.')
            self.cuda = False
        ...
        print(f'{self.model_path} model loaded on {device}. Classes loaded: {self.num_classes}.')
    ...
    def detect_image(self, image):
        ...
        for i, c in list(enumerate(top_label)):
            ...
            # Keep score as plain float to avoid unnecessary device moves
            out.append((x1, y1, x2, y2, predicted_class, float(score)))
