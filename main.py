...
def main(arg):
    ...
    # interactive: mouse callback on existing window
    scale_x = 1.0
    scale_y = 1.0
    ...
    def _on_mouse(event, x, y, flags, userdata):
        ...
        if scale_x <= 0 or scale_y <= 0:
            print("[CLICK] Ignored: invalid scale factors.")
            return
        ox = int(x / scale_x)
        oy = int(y / scale_y)
        try:
            boxes = DRA.get_last_boxes()
        except Exception as e:
            print(f"[CLICK] No boxes available: {e}")
            return
        hit = None
        best_area = None
        for b in boxes:
            ...
            if x1 <= ox <= x2 and y1 <= oy <= y2:
                ...
                if hit is None or area < best_area:
                    hit = b
                    best_area = area
        if hit is None:
            print(f"[CLICK] ({x},{y}) -> ({ox},{oy}) no hit")
            return
        tid = hit['id']
        DRA.toggle_visibility(tid)
        print(f"[CLICK] ({x},{y}) -> ({ox},{oy}) toggled id={tid}")
